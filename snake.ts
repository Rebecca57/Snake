...
class GameOver extends HTMLElement{
    ...
    constructor(){
        ...
        const template = document.getElementById('modelGameOver') as HTMLTemplateElement;
...
//Global values
let eatScore = 50 as number;
let boxSize = 15 as number;
let defaultDirection = "up" as string;
let snakeColor = "red" as string;
let appelColor ="green" as string;
let speed = 500;

//Canvas elements
let zoneJeu = document.querySelector('#zoneJeu') as HTMLCanvasElement;
let ctx = zoneJeu.getContext('2d') as CanvasRenderingContext2D;
...
let start:HTMLDivElement = document.getElementById('divStart') as HTMLDivElement;
...
class Game{
    ...
    private _apple :Box;
    private _appleLeft: number;
    private _appleTop: number;
    private _possibleApplePositions : Array<Array<number>>;
    private _direction : string; //actual movement direction ['up','down','rigth','left']

    constructor(firstSnake:Snake,firstApple:Box,defaultDirection:string){
        ...
        //[[x0,y0],[x1,y1],......,[xn,yn]]
        this._possibleApplePositions = new Array<Array<number>>();
    ...
    removeCoord(coord:Array<number>):void{
        for (let i: number = 0; i<this._possibleApplePositions.length ;i++){
    ...
    addCoord(coord:Array<number>):void{
 ...
 //class Snake
 class Snake{
    private _list: Array<Box>;
    private _left: number;
    private _top: number;
    private _score: number;

    constructor(firstBox: Box,score:number=0){
        this._list = new Array<Box>;
 ...
 class Box{
    ...
    constructor(size:number,left:number,top:number,color:string){
        this._size = size;
        this._left = left;
        this._top = top;
        this._color = color;
...
''
 ...
 function listenArrow(event:any,game: Game):void{
